...
main <- function(
  ...
  assert_that(all(
    colnames(contrasts_df) == c("contrast_name", "group1", "group2")
  ))
...
check_contrast <- function(contrast_name, group1, group2, sample_names) {
  ...
  assert_that(
    ...
    msg = glue(
      "All samples in {contrast_name} must be in the sample sheet. Extra samples found: {paste(extra_samples, collapse = ',')}"
    )
  )
  ...
  assert_that(
    ...
    msg = glue(
      "Each sample can only appear in one group per contrast (check {contrast_name})"
    )
  )
}
...
write_version <- function(version_file, process_name = "${task.process}") {
  readr::write_lines(
    glue('"{process_name}":', "R: {get_r_version()}", .sep = "\\n\\t"),
    version_file
  )
}
