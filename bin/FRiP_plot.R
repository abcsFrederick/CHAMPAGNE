...
merge_files <- function(folder) {
  files <- list.files(
    path = paste0(folder, "/PeakQC"), pattern = "FRiP_table.txt",
    full.names = T
  )
  allList <- lapply(files, read.table, header = T)
  allData <- do.call(rbind.data.frame, allList)
  write.table(allData, paste0(folder, "/PeakQC/FRiP_All_table.txt"),
    quote = F,
    row.names = F, sep = "\t"
  )
  return(allData)
...
plot_barplots <- function(inData, groupName, folder) {
  ...
  p <- p + geom_bar(position = "dodge", stat = "identity") +
    ...
    labs(
      title = groupName, x = "bam file", y = "Fraction of Reads in Peaks (FRiP)",
      fill = "peak file"
    )
...
plot_scatterplots <- function(inData, groupName, folder) {
  ...
  p <- p + geom_point(size = 2.5) +
    ...
    labs(
      title = groupName, x = "Number of Bases in Peaks (M)",
...
plot_barplots_self <- function(inData2, folder) {
  ...
  p <- p + geom_bar(position = "dodge", stat = "identity") +
    ...
    theme(axis.text.x = element_text(angle = -15, hjust = 0)) +
    labs(
      title = "All Samples", x = "bam file", y = "Fraction of Reads in Peaks (FRiP)",
      fill = "Group"
    )
...
plot_scatterplots_self <- function(inData2, folder) {
  ...
  p <- p + geom_point(size = 2.5) +
    ...
    annotation_logticks(sides = "b") +
    labs(
      title = "All samples", x = "Number of Bases in Peaks (M)",
      y = "Fraction of Reads in Peaks (FRiP)",
      shape = "peak file", color = "peak calling tool"
    )
