...
sample_frip_plot <- frip_dat %>%
  ...
  pivot_longer(
    c(`Fraction of Reads in Peaks`, `Number of Reads in Peaks (millions)`),
    names_to = "metric"
  ) %>%
  ...
  geom_hline(
    yintercept = seq(
      1.5,
      max(length(unique(frip_dat %>% pull(bedsample))) - 0.5, 1.5),
      1
    ),
    lwd = 0.5,
    color = "grey92"
...
ggsave(
  filename = "FRiP_samples.png",
  plot = sample_frip_plot,
  device = "png",
  dpi = 300,
  height = 4.5,
  width = 6.5
...
nbasesM_frip_plot <- frip_dat %>%
  ...
  labs(
    x = "Number of Bases (millions) in Peaks",
    y = "Fraction of Reads in Peaks"
  ) +
  theme_bw()
ggsave(
  filename = "FRiP_nbasesM.png",
  plot = nbasesM_frip_plot,
  device = "png",
  dpi = 300,
  height = 4,
  width = 6
